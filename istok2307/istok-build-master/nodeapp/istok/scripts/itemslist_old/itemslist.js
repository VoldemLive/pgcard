...
$(document).ready(function() {
  ...
  if (isQuantityOnPage != null) {
    ...
    $('.itemsQuantity').dropdown('set selected', '50');
    UpdateQueryString('page', '1');
    UpdateQueryString('limit', '50');
    filterQuantityIsReady = true;
  }
          ...
          $.each(data.data, function(index, item) {
            if (data.data[index].images != null && data.data[index].images.length > 0) {
              ...
              $('#itemsListTBody').append(
                '<tr>' +
                '<td>' + data.data[index].ourarticul + '</td>' +
                '<td>' + data.data[index].cnarticul + '</td>' +
                '<td>' + data.data[index].ourname + '</td>' +
                '<td class="item' + index + '"></td>' +
                '<td>' +
                '<div class="itemrowimage"><img class="ui medium image" data-images=' +
                JSON.stringify(imagesInJson) +
                ' onclick="getNextImage(this)" draggable="false" src="/api/v1/images/thumb/' +
                data.data[index].images[0] +
                '"></img><i data-images=' + JSON.stringify(imagesInJson) + ' onclick="openLightBox(this)" class="zoom-in huge icon zoompanel"></i></div></td>' +
                '<td><a href="/edititem/' + data.data[index].id + '"><i class="edit icon"></i></a></td>' +
                '</tr>');
            } else {
              $('#itemsListTBody').append(
                '<tr>' +
                ...
                '<td class="item' + index + '"></td>' +
                '<td>' +
                '<div class="itemrowimage"><img class="ui medium image" draggable="false" src="/api/v1/images/thumb/empty.jpg"></div></img></td>' +
                '<td><a href="/edititem/' + data.data[index].id + '"><i class="edit icon"></i></a></td>' +
                '</tr>');
            }
      ...
      success: function(data, textStatus, jqXHR) {
        if (data.data.length > 0) {
          for (var i = 0; i < data.data.length; i++) {
            $('.' + placeHolder).append('<div>' + replaceIdToName(data.data[i].partner) + ' : ' + data.data[i].quantity + '</div>');
          }
        } else {
